...
function Bucket(local_bucket, bucket) {
  ...
  this.manifest_file = path.join(local_bucket, MANIFEST_FILE);
...
_.extend(Bucket.prototype, {
  push: function push() {
    ...
    return self.get_remote_manifest().catch(function(reason) {
      ...
      return self.get_local_manifest().then(function(local) {
        return [remote, local];
      });
    }).spread(function(remote, local) {
      var upload = remote ? self.upload_diff(remote, local) : self.upload_all(local);

      return upload.then(function() {
        self.put_remote_manifest(local);
      });
  ...
  put_remote_manifest: function put_remote_manifest(manifest) {
    ...
    var push = this.manifest_store.put(MANIFEST_FILE, body);
    var save = Q.ninvoke(fs, 'writeFile', this.manifest_file, body);
