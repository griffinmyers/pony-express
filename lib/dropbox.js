...
function Dropbox(access_key, destination, api_endpoint, content_endpoint, logger_prefix) {
  this.access_key = access_key;
  this.destination = destination;
  this.cursor_file = path.join(this.destination, TOKEN_FILE);
  this.api_endpoint = api_endpoint;
  this.content_endpoint = content_endpoint;
  this.logger_prefix = logger_prefix || 'dropbox';
}
...
_.extend(Dropbox.prototype, {
  ...
  sync_folder: function sync_folder(entry) {
    ...
    // ## sync_folder
    //
    // Removes an existing folder and all its contents and deeply creates
    // the folder structure passed.
    //
    var local_path = path.join(this.destination, entry.path_lower);

    this.log('info', 'Creating', local_path);
  ...
  sync_delete: function sync_delete(entry) {
    ...
    // ## sync_delete
    //
    // Deletes the entity locally.
    //
    var local_path = path.join(this.destination, entry.path_lower);

    this.log('info', 'Deleting', local_path)
  ...
  sync_file: function sync_file(entry) {
    ...
    this.log('info', 'Syncing', local_path)
  ...
  list_folder_req: function list_folder_req(cursor) {
    ...
    // # list_folder_req
    //
    // Makes a request for metadata associated with a folder (the root, in all
    // cases we're interested in).  If a cursor is provided, it is uses to
    // return only a delta of changes since the last cursor.  Otherwise, all
    // root metadata is returned.
    //
    var endpoint;
  ...
  api_req: function api_req(pathname, options) {
    ...
    //
    return this.req(pathname, this.api_endpoint, options);
  },
  api_content_req: function api_content_req(pathname, file_pathname) {
    ...
    //
    return this.req(pathname, this.content_endpoint, {
      encoding: null,
  ...
  req: function req(pathname, endpoint, options) {
    //
    // ## req
    //
