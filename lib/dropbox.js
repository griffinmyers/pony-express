...
_.extend(Dropbox.prototype, {
  ...
  sync_entries: function sync_entries(result) {
    ...
    return Q.all(_.map(folder_or_delete[true], self.sync_folder_or_delete.bind(self))).then(function() {
      return Q.all(_.map(folder_or_delete[false], self.sync_file.bind(self)));
  ...
  delta: function delta(cursor) {
    ...
    return self.delta_req(cursor).then(function(result) {
      if(result.has_more) {
        return self.delta(cursor).then(function(all_result) {
  ...
  api_req: function api_req(pathname, options) {
    ...
    //
    return this.req(pathname, 'api.dropbox.com', options).spread(function(body) {
      return body;
    });
  },
  api_content_req: function api_content_req(pathname) {
    ...
    return this.req(pathname, 'api-content.dropbox.com', {
      ...
      if(headers['content-type'] === 'application/json') {
        var parsed_body = JSON.parse(body.toString('utf8'));
